...
/*
    2D segment tree
    from tatyam's library https://github.com/tatyam-prime/kyopro_library/blob/master/2DSegmentTree.cpp
    usage:
    vector<pair<pair<int, int>, int>> pts(n); // pair<{x, y}, cnt>
    SegmentTree2D<int, add, zero> grid(pts);
*/
template<class T, T(*op)(T, T), T(*e)()>
struct SegmentTree {
    int size;
    vector<T> seg;
    SegmentTree(vector<T> v) : size((int)v.size()), seg(std::move(v)) {
...
struct SegmentTree2D {
    ...
    void set(key_type key, T val) {
        auto& X = keys[0];
        int i = int(lower_bound(X.begin(), X.end(), key) - X.begin());
    ...
    T get_inner(int d, int i, key_type y1, key_type y2) const {
        auto& Y = keys[d];
        int l = i << d, r = (i + 1) << d;
        l = int(lower_bound(Y.begin() + l, Y.begin() + r, y1, comp_y) - Y.begin());
        r = int(upper_bound(Y.begin() + l, Y.begin() + r, y2, comp_y) - Y.begin());
...
int main() {
    /* tested on https://codeforces.com/contest/1974/submission/264178035 */
    vector<pair<pair<int, int>, int>> vals = {
        {{0, 0}, 1},
        {{1, 3}, 4},
    };
    SegmentTree2D<int, add, zero> twod(vals);
    cout << twod.get(0, 0, 0, 0) << "\n";
    cout << twod.get(0, 1, 0, 3) << "\n";
    cout << twod.get(0, 5, 0, 5) << "\n";
}
